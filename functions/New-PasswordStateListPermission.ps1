Function New-PasswordStateListPermission
{
  ...
  Param
  (
    [Parameter(Mandatory = $true, ValueFromPipeline = $true, ValueFromPipelineByPropertyName = $true, Position = 0)][ValidateNotNullOrEmpty()][int32]$PasswordListID,
  ...
  Process
  {
    ...
    If ($PSCmdlet.ShouldProcess("PasswordListID $($PasswordListID) - Permission $($Permission)"))
    {
      Try
      {
        New-PasswordStateResource -uri "/passwordlistpermissions/$($uri)" -body "$($Body | ConvertTo-Json)" -method POST
      }
      Catch
      {
        Throw $_.Exception
