Function Remove-PasswordStateListPermission
{
  ...
  Process
  {
    ...
    If ($PSCmdlet.ShouldProcess("PasswordListID $($PasswordListID)"))
    {
      Try
      {
        Set-PasswordStateResource -uri "/passwordlistpermissions/$($uri)" -body "$($Body | ConvertTo-Json)" -method DELETE
      }
      Catch
      {
        Throw $_.Exception
