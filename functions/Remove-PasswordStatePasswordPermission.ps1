Function Remove-PasswordStatePasswordPermission
{
  ...
  (
    ...
    [Parameter(ValueFromPipelineByPropertyName = $true, Position = 1, HelpMessage = 'M for Modify or V for View permissions')][ValidateSet('M', 'V')][string]$Permission,
  ...
  Process
  {
    ...
    If ($PSCmdlet.ShouldProcess("PasswordID $($PasswordID)"))
    {
      Try
      {
        Set-PasswordStateResource -uri "/passwordpermissions/$($uri)" -body "$($Body | ConvertTo-Json)" -method DELETE
      }
      Catch
      {
        Throw $_.Exception
